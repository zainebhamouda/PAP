...
function Modal({ projet, segments, onClose, onSave }) {
  const { t } = useTranslation();
  const isEdit = !!projet?.id;
  const [form, setForm]     = useState({ nom: projet?.nom || '', description: projet?.description || '', segmentId: projet?.segmentId || '' });
  const [saving, setSaving] = useState(false);
  const [error, setError]   = useState('');

  const save = async () => {
    if (!form.nom.trim() || !form.segmentId) { setError(t('projects.modal.errors.nameAndSegmentRequired')); return; }
    setSaving(true); setError('');
    try {
      isEdit ? await adminAPI.updateProjet(projet.id, form) : await adminAPI.createProjet(form);
      onSave();
    } catch (e) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(11,30,61,.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, backdropFilter:'blur(4px)' }}>
      <div style={{ background:'#fff', borderRadius:18, width:'100%', maxWidth:440, overflow:'hidden', boxShadow:'0 24px 64px rgba(0,0,0,.2)' }}>
        ...
        <div style={{ background:'#92400E', padding:'1.4rem 1.75rem', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <p style={{ margin:0, fontSize:'.92rem', fontWeight:800, color:'#fff' }}>{isEdit ? t('projects.modal.editTitle') : t('projects.modal.createTitle')}</p>
        ...
        <div style={{ padding:'1.75rem' }}>
          ...
          <div style={{ marginBottom:'1.1rem' }}>
            ...
            <input type="text" value={form.nom} placeholder={t('projects.fields.namePlaceholder')}
              ...
              onFocus={e => { e.target.style.borderColor='#92400E'; e.target.style.boxShadow='0 0 0 3px rgba(146,64,14,.08)'; }}
          ...
          <div style={{ marginBottom:'1.1rem' }}>
            <label style={{ display:'block', fontSize:'.75rem', fontWeight:700, color:'#374151', marginBottom:6, textTransform:'uppercase', letterSpacing:'.06em' }}>
              {t('projects.fields.segment')}
            </label>
            <select value={form.segmentId} onChange={e => setForm(p => ({ ...p, segmentId: e.target.value }))}
              ...
              <option value="">{t('projects.fields.segmentDefault')}</option>
              {segments.map(s => <option key={s.id} value={s.id}>{s.nom} {s.plantNom ? `(${s.plantNom})` : ''}</option>)}
            </select>
          </div>

          <div style={{ marginBottom:'1.1rem' }}>
            <label style={{ display:'block', fontSize:'.75rem', fontWeight:700, color:'#374151', marginBottom:6, textTransform:'uppercase', letterSpacing:'.06em' }}>
              {t('projects.fields.description')} <span style={{ color:'#94A3B8', fontWeight:400, textTransform:'none' }}>{t('common.optional')}</span>
            </label>
            <textarea value={form.description} placeholder={t('projects.fields.descriptionPlaceholder')} rows={3}
          ...
          <div style={{ display:'flex', gap:9, justifyContent:'flex-end', marginTop:'1.4rem' }}>
            ...
            <button onClick={save} disabled={saving} style={{ padding:'9px 22px', borderRadius:10, border:'none', background:'#92400E', color:'#fff', fontWeight:700, fontSize:'.85rem', cursor:saving?'not-allowed':'pointer', opacity:saving?0.7:1, display:'flex', alignItems:'center', gap:7 }}>
...
export default function AdminProjets() {
  ...
  const segmentNom = searchParams.get('segmentNom');
  ...
  const load = useCallback(() => {
    setLoading(true);
    Promise.all([adminAPI.getProjets(), adminAPI.getSegments()])
      .then(([p, s]) => { setProjets(p.data); setSegments(s.data); })
  ...
  const del = async p => {
    if (!window.confirm(t('projects.toast.confirmDelete', { name: p.nom }))) return;
    try { await adminAPI.deleteProjet(p.id); load(); toast$(`"${p.nom}" ${t('projects.toast.deleted')}`); }
    catch { toast$(t('projects.toast.deleteError'), false); }
  };

  const saved = () => {
    setModal(null); load();
    toast$(modal?.id ? t('projects.toast.updated') : t('projects.toast.created'));
  };

  const filtered = projets.filter(p => {
    const matchSearch  = p.nom?.toLowerCase().includes(search.toLowerCase());
    const matchSegment = filterSegment ? String(p.segmentId) === String(filterSegment) : true;
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1.4rem' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        {segmentId && (
          <button onClick={() => navigate('/admin/segments')}
            ...
            {IC.back} {t('projects.backToSegments')}
          </button>
        )}
        <div style={{ position:'relative', flex:1 }}>
          <span style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', color:'#94A3B8' }}>{IC.search}</span>
          <input type="text" placeholder={segmentNom ? `${t('projects.search.placeholder').replace('…','')} ${segmentNom}…` : t('projects.search.placeholder')}
...
<option value="">{t('filtrage.segments')}</option>
          {segments.map(s => <option key={s.id} value={s.id}>{s.nom} {s.plantNom ? `(${s.plantNom})` : ''}</option>)}
        </select>
        <button onClick={() => setModal(segmentId ? { segmentId: parseInt(segmentId) } : {})}
          style={{ display:'flex', alignItems:'center', gap:7, background:'#92400E', color:'#fff', border:'none', borderRadius:12, padding:'10px 20px', fontWeight:700, fontSize:'.85rem', cursor:'pointer', whiteSpace:'nowrap' }}>
          {IC.plus} {t('projects.newButton')}
        </button>
      </div>

      {loading ? (
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', padding:'3.5rem', gap:12, color:'#94A3B8' }}>
          <span style={{ width:26, height:26, border:'3px solid #E2E8F0', borderTopColor:'#92400E', borderRadius:'50%', animation:'spin .8s linear infinite', display:'inline-block' }}/> {t('common.loading')}
        </div>
      ) : (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'1rem' }}>
          {filtered.map((p, i) => (
            <div key={p.id} className="sc" style={{ background:'#fff', borderRadius:18, boxShadow:'0 2px 16px rgba(0,0,0,.07)', border:'1px solid #D1D5DB', animation:`up .4s ${i*.05}s ease both`, padding:'1.4rem' }}>
              <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:'1rem' }}>
                <div style={{ width:48, height:48, borderRadius:14, background:'linear-gradient(135deg,#92400E,#D97706)', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.2rem', fontWeight:900 }}>
                  {p.nom?.[0]?.toUpperCase()}
                </div>
                <div>
                  <p style={{ margin:0, fontWeight:800, fontSize:'.92rem', color:'#0B1E3D' }}>{p.nom}</p>
                  <p style={{ margin:'3px 0 0', fontSize:'.74rem', color:'#94A3B8' }}>{p.segmentNom || '—'}</p>
                </div>
              </div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:4, background:'#F8FAFC', borderRadius:8, padding:'6px 10px', marginBottom:'1rem', fontSize:'.72rem', color:'#64748b', fontWeight:600, flexWrap:'wrap' }}>
                <span>{p.siteNom||'—'}</span><span style={{ color:'#CBD5E1' }}>›</span>
                <span>{p.plantNom||'—'}</span><span style={{ color:'#CBD5E1' }}>›</span>
                <span>{p.segmentNom||'—'}</span><span style={{ color:'#CBD5E1' }}>›</span>
                <span style={{ color:'#92400E' }}>{p.nom}</span>
              </div>
              ...
              <div style={{ display:'flex', gap:8 }}>
                <button className="ib" onClick={() => navigate(`/admin/series?projetId=${p.id}&projetNom=${encodeURIComponent(p.nom)}`)}
                  style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:5, padding:'8px 0', borderRadius:15, border:'none', background:'#cdcdcd', color:'#07143e', fontSize:'.78rem', fontWeight:700, cursor:'pointer' }}>
                  {t('projects.series')} {IC.arrow}
                </button>
          ...
          {!filtered.length && (
            <div style={{ gridColumn:'1/-1', textAlign:'center', padding:'3.5rem', color:'#94A3B8' }}>
              <p style={{ fontWeight:600 }}>{t('projects.notFound')}</p>
            </div>
          )}
        </div>
      )}
      {modal !== null && <Modal projet={modal?.id ? modal : null} segments={segments} onClose={() => setModal(null)} onSave={saved} />}
    </div>
  );
}